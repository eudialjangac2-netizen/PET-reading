/* =====================================================================
   RENDERER - PART 5 (article cloze + MCQ 4 lựa chọn, split-pane, chỉ highlight bên trái)
   Schema JSON kỳ vọng:
   {
     partType: "part5",
...
function renderPart5(data, theme) {
  const titleEl = document.getElementById("appTitle");
  const emoji = theme ? theme.titleEmoji : "";
  titleEl.textContent = (emoji ? emoji + " " : "") + data.exerciseName;

  const loginSubtitle = document.getElementById("loginSubtitle");
  if (loginSubtitle && theme) loginSubtitle.textContent = `PET Reading Part 5 - ${theme.mascot}`;
...
registerRenderer("part5", renderPart5);
